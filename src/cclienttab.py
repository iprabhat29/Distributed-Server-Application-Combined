...
def Client1(num):
  while True:
    ...
    data = s.recv(1024)
    # time.sleep(10)
    print "Sending Request from Client"+str(num)+"-->", message1
    s.send(message1.encode('ascii'))
    data = s.recv(1024)
    if data:
      print "DATA RECEIVED from FEP by client"+str(num)+"--->", data
      #data = s.recv(1024)
      #if data:
...
def Client2(num):
  while True:
    ...
    data = s.recv(1024)
    if data:
      print "DATA RECEIVED by client"+str(num)+" from FEP--->", data
      print 'Time taken for processing request', time.time() - start_time
      # time.sleep(1)
    s.close()
#===============================================================================
...
def startClient(num,val):
    print 'In Start Client Function'
    if num==1:
        t = threading.Thread(target=Client1, args=(val,))
    elif num==2:
        t = threading.Thread(target=Client2, args=(val,))
    elif num==3:
        t = threading.Thread(target=Client3, args=(val,))
    elif num==4:
        t = threading.Thread(target=Client4, args=(val,))
    elif num==5:
        t = threading.Thread(target=Client5, args=(val,))
    else:
        t = threading.Thread(target=Client6, args=(val,))
      
    t.start()  

if __name__ == '__main__':
    text = raw_input("Enter number of clients")
    text=int(text)
    Main(text)
