    ...
    # print "DATA rcvd from FEP1--->",data
        ...
        # print "CLOCK--------->",data_temp[0]
          ...
          # print "SENDING ACK_FEP2 TO FEP1"
                    ...
                    data_temp = data.split(':')
                    float_temp_clock = float(data_temp[0])
                    float_temp_clock = max(localCLKFEP2,
                            float_temp_clock)
                    float_temp_clock += 0.0001
...
# ===============================================================================
# THIS IS MODULES RUNS AS HEARBEAT SERVER FOR FEP1 SERVER
# ===============================================================================


def FEP1_Heatbeat_socket(num1):
    print '---------Starting FEP1 Hearbeat Socket'
 ...
 # fcntl.fcntl(s, fcntl.F_SETFL, os.O_NONBLOCK)
    ...
    s.bind((HOST, PORT))
    s.listen(1)
    print '------FEP1 Heartbeat SOCKET LISTNING ON PORT 9093'
    while 1:       
        conn, addr = s.accept()
        print '----------------------Heartbeat check from Dispatcher----------------------', addr
        data = conn.recv(1024)
        if data:
            conn.send(str('Heartbeat'))
...
# ===============================================================================
# THIS IS MODULES RUNS AS HEARBEAT SERVER FOR FEP2 SERVER
# ===============================================================================


def FEP2_Heatbeat_socket(num1):
    print '---------Starting FEP2 Hearbeat Socket'
    HOST = 'localhost'  # Symbolic name meaning the local host
    PORT = 9095  # Arbitrary non-privileged port

    
    print '------FEP2 Heartbeat SOCKET LISTNING ON PORT 9093'
    counter=0
    while 1:
        counter=counter+1
        if counter<100: 
            ...
            conn, addr = s.accept()
            print '----------------------Heartbeat check from Dispatcher to FEP 2----------------------', counter
            data = conn.recv(1024)
            if data:
                conn.send(str('Heartbeat'))
        else:
            print '----------------------No heartbeat from FEP 2 to Dispatcher ----------------------', counter
            s.close()
            time.sleep(60)
            counter = 0
...
def FEP1(num):
    ...
    if result == 0:
        ...
        print 'Starting FEP 1 heartbeat server'
        start_new_thread(FEP1_Heatbeat_socket, (1,))
...
#
#     s1.connect((HOST, PORT))
#     print ('CONNECTED TO PORT 9093 FROM FEP2')
#
#   except Exception, e:
#     print str(e)
#     print 'Not working'
...
result
                            ...
                            if 'incrementMedalTally' in data \
                                or 'setScore' in data:
...
in cache_dic.keys():
                                    temp_list = []
                                    temp_list = \
list(cache_dic[data_hash_temp])
                                    temp_list[1] = 1
                                    cache_dic[data_hash_temp] = \
temp_list
                                    print 'F1:****ADDING 1 TO DIRTY BIT******', \
cache_dic[data_hash_temp]
                            else:
                                result = EndServer(data)
                                print 'F1*******CACHE MISS***********'
                                if len(cache_dic) < 5:
                                    ...
                                    print 'F1:***********ADDING ELEMENT INTO CACHE*********', \
cache_dic[data_hash]
                                else:
                                    ...
                                    r_list.append(0)
                                    cache_dic[data_hash] = r_list
                                    print 'F1:****ADDING NEW ELEMENT IN CACHE******', \
cache_dic[data_hash]
                    else:
                        print '############## CLIENT POLL MODE IS ON################'
                        poll = poll + 1
                        data_hash = hash.encode(data)
                        if data_hash in cache_dic.keys():
                            result_1 = []
                            ...
                            if result_1[1] == 0:
                                print 'F1:****CACHE HIT******'
                                result = result_1[0]
                                print 'F1:********FROM LOCAL CACHE--->', \
result
                            else:
                                print 'F1:MODE =1 ************RESULT GETTING FROM ENDSERVER'
                                result = EndServer(data)
                        else:
                            if 'incrementMedalTally' in data \
                                or 'setScore' in data:
...
temp_list
                                    print 'F1:****ADDING 1 TO DIRTY BIT******', \
cache_dic[data_hash_temp], result
                            else:
                                result = EndServer(data)
                                print 'POLL--->', poll
                                print 'F1*******CACHE MISS***********'
...
cache_dic[data_hash]
                        ...
                        if poll % 3 == 0:
                            arr_index = []
                            print 'F1:*******MODE 1 REMOVING ELEMENT FROM CACHE*********'
                            for i in cache_dic:
                                res = cache_dic[i]
                                if res[1] == 1:
                                    print 'F1----> MODE = 1 Deleting stale data'
                                    arr_index.append(i)
                            print "Deleting Dictionary", arr_index
                            for i in arr_index:
                                if i in cache_dic.keys():
                                    del cache_dic[i]
...
def FEP2(num):
    ...
    if result == 0:
        ...
        print 'Starting FEP 2 heartbeat server'
        start_new_thread(FEP2_Heatbeat_socket, (1,))
                    ...
                    if mode == 0:
                        ...
                        if data_hash in cache_dic.keys():
                            ...
                            if result_1[1] == 0:
                                print 'F2:****CACHE HIT******'
                                result = result_1[0]
                                print 'F2:********FROM LOCAL CACHE--->', \
result
                            else:
                                result = EndServer(data)
                                print 'F2:************Removing DIRTY RESULT FROM CACHE**********'
                                del cache_dic[data_hash]
                        else:
                            if 'incrementMedalTally' in data \
                                or 'setScore' in data:
...
list(cache_dic[data_hash_temp])
                                    ...
                                    cache_dic[data_hash_temp] = \
temp_list
                                    print 'F2:****ADDING 1 TO DIRTY BIT******', \
cache_dic[data_hash_temp], result
                            else:
                                result = EndServer(data)
                                print 'F2*******CACHE MISS***********'
                                if len(cache_dic) < 5:
                                    ...
                                    r_list.append(0)
                                    cache_dic[data_hash] = r_list
                                    print 'F2:***********ADDING ELEMENT INTO CACHE*********', \
cache_dic[data_hash]
                                else:
                                    ...
                                    r_list.append(0)
                                    cache_dic[data_hash] = r_list
                                    print 'F2:****ADDING NEW ELEMENT IN CACHE******', \
cache_dic[data_hash]
                    else:

        # result = EndServer(data)

                        print '############## CLIENT PUSH MODE IS ON################'
                        poll = poll + 1
                        data_hash = hash.encode(data)
                        if data_hash in cache_dic.keys():
                            result_1 = []
...
result
                                ...
                                result = EndServer(data)
                                print 'F2:MODE =1 ************RESULT GETTING FROM ENDSERVER'
                                result = EndServer(data)
                        else:
                            if 'incrementMedalTally' in data \
                                or 'setScore' in data:
...
cache_dic[data_hash]
                        ...
                        if poll % 3 == 0:
                            ...
                            for i in cache_dic:
                                res = cache_dic[i]
                                if res[1] == 1:
                                    print '$$$$$$$$$$$$$$$$$F2----> MODE = 1 Deleting stale data'
                                    print len(cache_dic)
                                    arr_index.append(i)
                            print "F2-------Deleting Dictionary", arr_index
                            for i in arr_index:
                                if i in cache_dic.keys():
                                    del cache_dic[i]
...
# ===============================================================================
# THIS THREAD IS CALLED FOR FEP 2 SERVER EVERY 10 SECONDS TO CHECK FOR ITS 
# AVAILABILITY.IN CASE THERE IS NO RESPONSE THE GLOBAL VARIABLE FEP2_heartbeats IS 
# RESET AND THE CLIENTS ARE DISPATCHED TO FEP 1 SERVER.
# ===============================================================================
...
def FEP2_heartbeat():
    ...
    s1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print 'Dispatcher checking with FEP2 Server'
    try:
        s1.connect((HOST, PORT))
        s1.send('Check')
        data = s1.recv(1024)
        ...
        if data:
            print '^^^^^^^^^^^^^^^^^^^^Dispatcher got heartbeat from FEP 2^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^', data
            FEP2_heartbeats = 1
        else:
            print '^^^^^^^^^^^^^^^^^^^^NO  heartbeat from FEP 2^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^'
            FEP2_heartbeats=0
    except:
        print '^^^^^^^^^^^^^^^^^^^^NO heartbeat from FEP 2^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^'
        FEP2_heartbeats = 0
...
# ===============================================================================
# THIS THREAD IS CALLED FOR FEP 1 SERVER EVERY 10 SECONDS TO CHECK FOR ITS 
# AVAILABILITY.IN CASE THERE IS NO RESPONSE THE GLOBAL VARIABLE FEP1_heartbeats IS 
# RESET AND THE CLIENTS ARE DISPATCHED TO FEP 2 SERVER.
# ===============================================================================
...
def FEP1_heartbeat():
    ...
    s1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print 'Dispatcher checking with FEP1 Server'
    try:
        s1.connect((HOST, PORT))
        s1.send('Check')
        data = s1.recv(1024)
        ...
        if data:
            print '^^^^^^^^^^^^^^^^^^^^Dispatcher got heartbeat from FEP 1^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^', data
            FEP1_heartbeats = 1
        else:
            FEP1_heartbeats=0
    except:
...
def WorkDispatcher(num):
    ...
    while True:
        ...
        if ClientLoad.qsize() > 0:
            FEP1_status=FEP1_heartbeat()
            FEP2_status=FEP2_heartbeat()
            print 'FEP2_Status',FEP2_status
            if FEP1_status == 1 and FEP2_status == 1:
                ...
                if i == 2:   
                    ...
                    i = 0
                    cv.notify_all()
            elif FEP1_status == 1 and FEP2_status== 0:
                print '@@@@@@@@@@@@@@@@@@@@@@@@@ FEP 2 SERVER DOWN @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
                FEP1Queue.put(ClientLoad.get())
                cv.notify_all()
                
            elif FEP1_status == 0 and FEP2_status == 1:
                print '@@@@@@@@@@@@@@@@@@@@@@@@@ FEP 1 SERVER DOWN @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
                FEP2Queue.put(ClientLoad.get())
                cv.notify_all()
        else:
            cv.wait()
...
if __name__ == '__main__':
    ...
    #start_new_thread(FEP1_heartbeat, (1,))
    #start_new_thread(FEP2_heartbeat, (1,))
 ...
 # start_new_thread(startserver2, (1,))
    ...
    t = threading.Thread(target=FEP2, args=(1,))
    t.start()
    Main()
